...
// UTC = PST + 7
//deploy from 8pm to 6am = 3 to 13 
//deploy from 6pm to 9am = 1 to 15

//Hawaii time from 6pm to 6am = 4 to 16
int h_stt = 1;  
int h_end = 15;
int m_stt = 15;
int m_end = 25;
...
const int batt_th = 3.2;

void setup() {
  Serial.begin(9600);
  Serial.println("PUE Control Rev04c");
  Serial.println("Last update: Nov 8, 2019");
  //digital pin setup 
  ...
  //initialize MCP9808 for LED temp monitoring
  //mcp9808begin();
...
void loop() {
  // start loop by turning RTC on to check time
  digitalWrite(peri_en_pin,LOW);  //3.3V ON
  delay(500);
  setSyncProvider(RTC.get);
  //if(checkTimeMin(m_stt,m_end)){
  //if(checkTimeHour(h_stt,h_end&&battRead()>batt_th)){
  if(checkTimeHour(h_stt,h_end)){
    digitalWrite(signal_led_pin,HIGH);
    //check time; if it is deploy time
    if(pi_status==0){
      //first time turning Pi on
      turnonPi(); //turn Pi on
      delay(1000);
    }
    else{
  ...
  digitalWrite(signal_led_pin,LOW);
  printTime();
  //if it is NOT deploy time
  if(pi_status==1){
    //if Pi is ON, then turn off first
    shutdownPi(100);
  }
  else{
    //if Pi is already off, then just sleep and use watchdog to wake every 8s
    blinkSignal(2);
    digitalWrite(peri_en_pin,HIGH); //turn peripherals off before going to sleep
...
void sendData(){
  ...
  // send MCP9808 data
  //tempsensor.wake();
  //Serial.println(tempsensor.readTempC()); //last one has to be println
  Serial.println(0);
...
void shutdownPi(int thres){
  ...
  delay(25000); //wait for Pi to shutdown
